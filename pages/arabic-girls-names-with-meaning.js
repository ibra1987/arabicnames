        ...
        {displayedNames &&
          displayedNames.map((name, index) => (
            <>
              <div
                key={name._id}
                className="w-full md:w-4/5 bg-gray-50 border-b flex justify-start items-start p-2"
              >
                <h2 className=" w-1/4">{name.Name}</h2>
                <span className="w-1/3 md:w-2/4">{name.Meaning}</span>
                <audio
                  controls
              ...
              {index % 10 === 0 && index !== 0 && (
                <div className="w-full md:w-4/5 bg-pink-100 h-48 my-10">
                  add
                </div>
              )}
            </>
